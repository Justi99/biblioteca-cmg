    ...
    public function registrar()
{
    ...
    if (empty($libro) || empty($estudiante) || empty($cantidad) || empty($fecha_prestamo) || empty($fecha_devolucion)) {
        ...
        if ($verificar_cant['cantidad'] >= $cantidad) {
            ...
            // 1. Primero verificamos si el modelo nos devolvió el aviso de "existe"
            if ($data == "existe") {
                $msg = array('msg' => 'El libro ya está prestado a este estudiante', 'icono' => 'warning');
            
            // 2. Luego verificamos si es un número válido mayor a 0 (Éxito real)
            } else if (is_numeric($data) && $data > 0) {
                $msg = array('msg' => 'Libro Prestado', 'icono' => 'success', 'id' => $data);
            
            // 3. Si no es ni lo uno ni lo otro, es error
            } else {
    ...
    public function ticked($id_prestamo)
{
    ...
    // Si el ID no es un número (ej: es "existe"), detenemos todo aquí.
    if (!is_numeric($id_prestamo)) {
        // Opcional: Puedes redirigir o mostrar mensaje simple
        echo "Error: ID de préstamo no válido.";
        die(); 
    }
    // ----------------------------------------
    ...
    // Ahora es seguro llamar a la base de datos porque sabemos que es un número
    $prestamo = $this->model->getPrestamoLibro($id_prestamo);

    if (empty($prestamo)) {
        header('Location: '.base_url. 'Configuracion/vacio');
        exit; // Es buena práctica poner exit después de header location
    }
